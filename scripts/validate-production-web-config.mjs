...
function required(environment, key) {
  ...
  if (!value || /replace-with|example|invalid/i.test(value)) failure(`${key} is missing or contains a placeholder`);
  return value;
}
...
function stringList(value, label) {
  ...
  if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== "string" || !item.trim())) failure(`${label} must be a JSON string array`);
...
function httpsUrl(value, label) {
  ...
  if (parsed.protocol !== "https:" || parsed.username || parsed.password || parsed.search || parsed.hash) failure(`${label} must be a valid HTTPS URL`);
  return parsed;
}
...
export function validateProductionWebConfig(environment) {
  ...
  if (!/^\/[A-Za-z0-9._/-]*\/$/.test(values.PORTAL_PUBLIC_BASE_PATH) || values.PORTAL_PUBLIC_BASE_PATH.includes("//")) failure("PORTAL_PUBLIC_BASE_PATH must be an absolute directory path");
  ...
  const functionsBaseUrl = httpsUrl(values.PORTAL_FUNCTIONS_BASE_URL, "PORTAL_FUNCTIONS_BASE_URL").toString().replace(/\/$/, "");
  const authorizedDomains = stringList(values.PORTAL_FIREBASE_AUTHORIZED_DOMAINS_JSON, "authorized domains");
  const allowedWebOrigins = stringList(values.PORTAL_ALLOWED_WEB_ORIGINS_JSON, "allowed web origins").map((origin) => httpsUrl(origin, "allowed web origins").origin);
  const pagesHostname = new URL(pagesOrigin).hostname.toLowerCase();
  if (!authorizedDomains.includes(pagesHostname) || !authorizedDomains.includes(authDomain)) failure("authorized domains must include the Pages and Firebase Auth hostnames");
  if (!allowedWebOrigins.includes(pagesOrigin)) failure("allowed web origins must include the Pages origin");
  return {
...
export async function verifyFirebaseAuthorizedDomains(config, fetchImpl = fetch) {
  const response = await fetchImpl(`https://identitytoolkit.googleapis.com/v1/projects?key=${encodeURIComponent(config.apiKey)}`);
  if (!response.ok) throw new Error(`Firebase authorized-domain check failed with HTTP ${response.status}`);
  const project = await response.json();
  if (project.projectId && project.projectId !== config.projectId) throw new Error(`Firebase project mismatch: expected ${config.projectId}, received ${project.projectId}`);
  const liveDomains = new Set(Array.isArray(project.authorizedDomains) ? project.authorizedDomains.map((value) => String(value).toLowerCase()) : []);
...
if (invokedAsScript) {
  try {
    ...
    if (process.argv.includes("--verify-authorized-domains")) await verifyFirebaseAuthorizedDomains(config);
